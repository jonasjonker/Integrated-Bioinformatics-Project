...
#seurat<- readRDS("C:\\Users\\user\\Desktop\\10xgenomics_data\\3ksorted\\seurat_3k_sorted.rds")
seurat
```
...
```{r}
feature_metadata <- fread("C:\\Users\\user\\Desktop\\10xgenomics_data\\3ksorted\\pbmc_granulocyte_sorted_3k_filtered_feature_bc_matrix\\filtered_feature_bc_matrix\\features.tsv.gz") %>%
...
```{r}
foo <- fread("C:\\Users\\user\\Desktop\\10xgenomics_data\\3ksorted\\pre_processed_pbmc_granulocyte_sorted_3k_atac_peak_annotation.tsv") %>%
  .[,c("peak","peak_type")] %>%
...
saveRDS(mofa,"C:\\Users\\user\\Desktop\\10xgenomics_data\\3ksorted\\mofa_3k_sorted.rds", compress = FALSE)
...
```{r}
plot_factor(mofa, factors=3, group_by = "celltype") +
  theme(
    axis.text.x = element_text(color="black", angle=40, vjust=1, hjust=1)
  )
```
...
```{r}
plot_factor(mofa, factors=3, group_by = "celltype", color_by="BANK1") +
  theme(
    axis.text.x = element_text(color="black", angle=40, vjust=1, hjust=1)
  )
```
...
```{r}
plot_factor(mofa, factors=2, group_by = "celltype", color_by="nFeature_ATAC") +
  theme(
    axis.text.x = element_text(color="black", angle=40, vjust=1, hjust=1)
  )
```
...
Can we inspect 2 factors together? We look now at Factors 1 and 2 this plot doesnt tell us much
```{r}
p <- plot_factors(mofa, 
  factors = c(1,1), 
  color_by = "PLXDC2",
...
```
It seems that most of the Factors do not have clear gene set signatures. A clear exception is Factor 5, which has a very strong enrichment for genes with negative weights and Factor 1 and 2.
```{r}
...
```{r}
plot_enrichment(gsea.negative, factor = 5, max.pathways = 15)
```
```{r}
plot_enrichment_detailed(
  enrichment.results = gsea.negative,
  factor = 5, 
  max.pathways = 3
)
```
